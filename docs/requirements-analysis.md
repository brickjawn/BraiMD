...
- **FR3: YAML Frontmatter Parsing.** The system must parse uploaded Markdown files to extract metadata — `name`, `description`, and `triggers` — from YAML frontmatter (via the `gray-matter` library) and store each field in its corresponding database column while preserving the raw Markdown body in the `content` column.
...
- **FR5: Agent API Endpoint (Extra Functionality).** The system must expose a RESTful API endpoint (`GET /api/skills?trigger=keyword`) allowing external AI agents to query and retrieve skill content dynamically. The endpoint uses MySQL's `JSON_CONTAINS` to match against the `triggers` JSON column. If the matched skill has a prerequisite (an inbound edge), the API returns a `prerequisite_required` response with the parent skill's content instead, enforcing the learning path.

- **FR6: Activity Logging.** The system must track and log when an AI agent accesses a specific skill, recording the `skill_id`, timestamp, outcome (`success` or `prerequisite_blocked`), `agent_id` (derived from the authenticated API key identity), and `client_ip` (the requester's IP address) in the `agent_logs` table. A dashboard view (`/dashboard/logs`) displays the 100 most recent log entries with all fields visible, enabling operators to distinguish queries from different agents and sources.
...
- **SR2: Database.** The database must be a relational MySQL 8.0 system, utilizing connection pooling via the `mysql2/promise` library. The schema comprises five tables: `users`, `skills`, `nodes`, `edges`, and `agent_logs`.
...
- **SR4: Container Runtime.** The application stack must be deployable via `podman-compose up -d`, producing two containers (`braimd-app` on port 3000, `braimd-db` on MySQL's default port) with the database initialized from `schema.sql` on first run.
...
**Answer:** AI agents need structured, predictable responses. When the agent hits the API endpoint with a search trigger — say `/api/skills?trigger=database_setup` — the server should return a standard JSON object. It needs a `status` indicator (like `"ok"` or `"not_found"`), the `skill_name`, and the raw, unrendered Markdown string in a `content` field so the LLM can read the step-by-step instructions directly. If no skill matches the trigger, return a clear `"not_found"` status so the agent can handle it gracefully without crashing. And every successful query should be logged automatically so I can see what skills are actually being used.
